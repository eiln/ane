...
#define tm_write32_rel(ane, off, val) \
	(writel_relaxed(val, ane->engine + ANE_TM_BASE + off))
#define tq_write32_rel(ane, off, val) \
	(writel_relaxed(val, ane->engine + ANE_TQ_BASE + off))

void ane_tm_enable(struct ane_device *ane)
{
	tm_write32_rel(ane, TM_TQ_EN, tm_read32(ane, TM_TQ_EN) | 0x1000);

	for (int qid = 0; qid < ANE_TQ_COUNT; qid++) {
		tq_write32_rel(ane, TQ_PRTY(qid), TQ_PRTY_TABLE[qid]);
	}

	tm_write32_rel(ane, TM_IRQ_EN1, 0x4000000);
...
int ane_tm_enqueue(struct ane_device *ane, struct ane_engine_req *req)
{
	...
	tq_write32_rel(ane, TQ_SIZE1(qid), ((req->td_size >> 2) - 1) << 0x10);
	tq_write32_rel(ane, TQ_ADDR1(qid), req->fifo_addr);
	tq_write32_rel(ane, TQ_NID1(qid), (req->nid & 0xff) << 8 | 1);
