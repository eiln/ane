...
class Model:
	def __init__(self, path):
		...
		self.outputs = [ctypes.create_string_buffer(tile_size(nchw)) for nchw in self.dst_nchw]
		self.inputs_pad, self.outputs_pad = [b''] * (TILE_COUNT - self.src_count), [b''] * (TILE_COUNT - self.dst_count)
